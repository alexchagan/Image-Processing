...
def blendDemo():
    ...
    im1 = cv2.cvtColor(cv2.imread('wallpaper.jpg'), cv2.COLOR_BGR2RGB) / 255
    im2 = cv2.cvtColor(cv2.imread('ball.jpg'), cv2.COLOR_BGR2RGB) / 255
    mask = cv2.cvtColor(cv2.imread('ball-mask.jpg'), cv2.COLOR_BGR2RGB) / 255
    ...
    cv2.imwrite('sky_ball.png', cv2.cvtColor((im_blend * 255).astype(np.uint8), cv2.COLOR_RGB2BGR))
